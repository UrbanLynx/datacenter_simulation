        ...
        for i in range(0, kval):
            for j in range(0, (kval/2)):
                for k in range (0, kval/2):
                    self.host.append(self.addHost('h%d%d%d' % (i, j, k), ip='10.%d.%d.%d/8' % (i, j, (k+2)), cls=Docker, dimage="aborase/simdocker:v4_fix"))
...
def start_varys(net):
    ...
    h000 = net.get('h000')
    result = h000.cmd('cd; cd group6/aalo1mb; ./bin/start-all.sh')
    print(result)
    ...
    for h in net.hosts:
        time.sleep(1)
        if h000 != h:
            result = h.cmd('cd; cd group6/aalo1mb; ./bin/varys-daemon.sh start varys.framework.slave.Slave varys://10.0.0.2:1606')
            print(result)
        #if h210 == h:
        #    break
