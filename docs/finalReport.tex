...
Recently published coflow schedulers Varys \cite{varys} and Aalo \cite{aalo} focus on minimizing coflow completion time (CCT) as opposed to individual flow completion time. Varys and Aalo were shown to decrease average CCT by a factor of about 3 or 2 times, respectively. However, both require the flows in a coflow to be identified to the scheduler via an API. This is a programmer burden, and may not be realistic in all cases: for example with legacy code, or there may be situations in which not all flows in a coflow are known to the programmer. Automatic coflow identification would be useful to address these issues. Such algorithms would likely not be one hundred percent accurate. This project thus attempts to examine the impact of removing individual flow labels on coflow scheduler performance.
